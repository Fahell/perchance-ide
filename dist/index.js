...
import { renderPanel, renderSetup } from "./ui/index.js";
...
function loadApiKey() {
    return storageGet(API_KEY_STORAGE) ?? null;
}
...
async function handleSendMessage(text, signal) {
    ...
    const response = await agentLoop(text, agentContext, (status) => {
            ...
            import("./browserpod/manager.js").then(({ browserPodManager }) => {
                browserPodManager.boot({ apiKey: bpKey, nodeVersion: "22" }).then((ok) => {
                    if (ok) {
                        ...
                        syncVfsToPod(false).catch((err) => console.warn("[Agent] Initial VFS→Pod sync failed:", err));
                    }
                    else {
