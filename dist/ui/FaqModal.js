...
function renderContent(text) {
    ...
    lines.forEach((line, i) => {
        ...
        if (trimmed === "") {
            ...
            elements.push(_jsx("div", { style: { height: "6px" } }, i));
        }
        else if (trimmed.startsWith("- ")) {
            inList = true;
            elements.push(_jsxs("div", { style: { display: "flex", gap: "6px", marginBottom: "4px", lineHeight: "1.5" }, children: [_jsx("span", { style: { color: colors.textMuted }, children: "\u2022" }), _jsx("span", { style: { color: colors.text, fontSize: "10px", fontFamily: fonts.mono, lineHeight: "1.5" }, children: trimmed.slice(2) })] }, i));
        }
        else if (trimmed.endsWith(":") && !trimmed.includes("  ")) {
            // Sub-header line
            elements.push(_jsx("div", { style: { color: colors.textSecondary, fontSize: "11px", fontWeight: "600", marginTop: "8px", marginBottom: "4px", lineHeight: "1.4", fontFamily: fonts.mono }, children: trimmed }, i));
        }
        else if (trimmed.includes("  ") && !trimmed.startsWith("    ")) {
            // Definition-style lines with double space separator
            const parts = trimmed.split(/\s{2,}/);
            elements.push(_jsx("div", { style: { display: "flex", gap: "8px", marginBottom: "3px", lineHeight: "1.5" }, children: parts.length >= 2 ? (_jsxs(_Fragment, { children: [_jsx("span", { style: { color: colors.text, fontSize: "10px", fontFamily: fonts.mono, minWidth: "150px", fontWeight: "600" }, children: parts[0] }), _jsx("span", { style: { color: colors.textMuted, fontSize: "10px", fontFamily: fonts.mono, lineHeight: "1.5" }, children: parts.slice(1).join("  ") })] })) : (_jsx("span", { style: { color: colors.text, fontSize: "10px", fontFamily: fonts.mono, lineHeight: "1.5" }, children: trimmed })) }, i));
        }
        else {
            inList = false;
            elements.push(_jsx("div", { style: { color: colors.text, fontSize: "10px", fontFamily: fonts.mono, lineHeight: "1.5", marginBottom: "2px" }, children: trimmed }, i));
        }
    });
    return elements;
}
// ─── Component ──────────────────────────────────────────────
export function FaqModal({ isOpen, locale, onClose }) {
    const [activeId, setActiveId] = useState(HELP_TOPICS[0].id);
    const activeTopic = HELP_TOPICS.find((t) => t.id === activeId) ?? HELP_TOPICS[0];
    return (_jsx(Modal, { isOpen: isOpen, onClose: onClose, title: t("faq.title", locale) || "help", wide: true, children: _jsxs("div", { style: {
                display: "flex",
                gap: "0",
                ...
                minHeight: "320px",
                        ...
                        overflowY: "auto",
                        flexShrink: 0,
                    }, children: HELP_TOPICS.map((topic) => (_jsx("div", { role: "button", tabIndex: 0, "aria-selected": topic.id === activeId, onClick: () => setActiveId(topic.id), onKeyDown: (e) => { if (e.key === "Enter" || e.key === " ") {
                            e.preventDefault();
                            ...
                            userSelect: "none",
                            lineHeight: "1.3",
                        }, children: topic.title }, topic.id))) }), _jsxs("div", { style: {
                        flex: "1",
                        padding: "8px 14px",
                        overflowY: "auto",
                        maxHeight: "400px",
                    }, children: [_jsx("div", { style: { color: colors.textSecondary, fontSize: "11px", fontWeight: "600", marginBottom: "8px", lineHeight: "1.4", fontFamily: fonts.mono }, children: activeTopic.title }), renderContent(activeTopic.content)] })] }) }));
}
