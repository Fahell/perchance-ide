...
export function AgentMessage({ message, agentStatus, compact, locale, onContinue, isLastAgentMessage }) {
    const isActive = message.role === "agent" && agentStatus !== "idle";
    return (_jsx("div", { style: { display: "flex", justifyContent: "flex-start" }, children: _jsxs("div", { style: { animation: "agent-slide-in 0.2s ease-out", maxWidth: message.toolCalls.length > 0 ? "100%" : "85%", textAlign: "left" }, children: [isActive && !message.content && message.toolCalls.length === 0 && _jsx(ThinkingIndicator, {}), message.toolCalls.map((tc) => (_jsx(ToolCallCard, { toolCall: tc }, tc.id))), !compact && _jsx(ResponseText, { content: message.content, loading: isActive && message.toolCalls.length > 0, locale: locale }), isLastAgentMessage && onContinue && agentStatus === "idle" && message.content && (_jsx("button", { onClick: () => onContinue(message.content), style: {
                        background: "none",
