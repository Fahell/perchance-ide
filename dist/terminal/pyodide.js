...
export async function getPyodide() {
    ...
    _loading = (async () => {
        try {
            ...
            const pyodideModule = await import(PYODIDE_MJS);
 ...
 */
export async function syncToPyodide() {
 ...
 */
export async function syncFromPyodide() {
    const pyodide = await getPyodide();
    let changed = 0;
...
function walkMemfs(pyodide, dir, _depth = 0, _visited = new Set()) {
    const results = [];
