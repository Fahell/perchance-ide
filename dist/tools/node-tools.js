 ...
 */
async function pullMetadataFromPod() {
...
function createRunNodeScriptTool() {
    return {
        ...
        async execute(toolArgs) {
            ...
            await syncVfsToPod();
            const scriptPath = toolArgs.path;
            const extraArgs = toolArgs.args?.trim();
...
function createExecuteNpmCommandTool() {
    return {
