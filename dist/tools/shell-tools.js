...
async function syncVfsToPod() {
    ...
    for (const cachedPath of cachedPaths) {
        if (!vfsFilePaths.has(cachedPath) && cachedPath.startsWith(PROJECT_ROOT + "/")) {
            ...
            if (ok)
                console.log(`[ShellTools] Deleted stale file on Pod: ${cachedPath}`);
        }
    }
}
 ...
 */
export async function pullProjectFilesFromPod() {
    ...
    try {
        ...
        for (const entry of vfsEntries) {
            if (entry.type !== "file")
                continue;
            if (!entry.path.startsWith(PROJECT_ROOT + "/"))
                continue;
            if (!isProjectSourceFile(entry.path))
                continue;
        ...
        if (pulled > 0 || sizeSkipped > 0 || orphaned > 0) {
            console.log(`[ShellTools] Pulled ${pulled} files from Pod → VFS (${skipped} filtered, ${sizeSkipped} oversized, ${orphaned} orphans removed)`);
        }
    }
    catch (err) {
...
function validateGitArgs(args) {
    if (args.length === 0)
        return "No git subcommand specified";
    const subcmd = args[0];
...
function createRunShellCommandTool() {
    return {
        ...
        description: "Execute a Bash shell command in the BrowserPod Linux environment. " +
...
function createRunGitCommandTool() {
    return {
        name: "run_git_command",
        description: "Execute a Git command in the BrowserPod environment. " +
        ...
        parameters: {
            args: {
                description: "Git subcommand and arguments (e.g., 'status', 'log --oneline -5', 'clone https://github.com/user/repo.git')",
                type: "string",
                required: true,
            },
        },
        ...
        async execute(toolArgs) {
            ...
            const rawArgs = toolArgs.args.trim();
            if (!rawArgs)
...
function createStartHttpServerTool() {
    return {
        name: "start_http_server",
        description: "Start an HTTP server inside the BrowserPod and expose it via a public portal URL. " +
        ...
        parameters: {
            command: {
                description: "The command to start the server (e.g., 'node server.js' or 'npx serve -l 3000')",
                type: "string",
                required: true,
            },
            port: {
        ...
        async execute(args) {
            ...
            const command = args.command.trim();
            if (!command)
                return "Error: No server command provided";
            ...
            const result = await Promise.race([
                serverPromise,
                new Promise((resolve) => setTimeout(() => resolve({ stdout: "", stderr: "Timeout waiting for server", exitCode: 1 }), 10_000)),
            ]);
