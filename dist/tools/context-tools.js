...
function searchMessages(messages, query, limit = 5) {
    const queryTerms = tokenize(query);
    if (queryTerms.length === 0)
        return [];
    const totalMessages = messages.length;
    // Phase 1: Pre-tokenize all messages once and build term-frequency maps
    const msgTermFreqs = new Array(totalMessages);
    const msgTermCounts = new Array(totalMessages);
...
export function createContextTools() {
    return {
        search_history: {
            name: "search_history",
            description: "Search your conversation history by keyword. USE this when the user references something from earlier in the conversation that is NOT in the recent messages above. Returns the most relevant matching messages with their positions.",
            parameters: {
            ...
            execute: async (args) => {
                ...
                const lines = results.map((r) => `[msg #${r.index}] ${r.author}: "${r.content.length > 200 ? r.content.slice(0, 200) + "..." : r.content}"`);
                return `Found ${results.length} relevant messages:\n${lines.join("\n")}`;
            },
        },
        get_messages: {
            name: "get_messages",
            description: "Get raw messages by position or count from your conversation history. USE this when you need exact quotes, full context, or specific messages. Returns formatted messages with author and position.",
            parameters: {
            ...
            execute: async (args) => {
                ...
                if (slice.length === 0)
                    return "No messages in the specified range.";
                const lines = slice.map((m, i) => `[${from + i}] ${m.author}: "${m.content.length > 300 ? m.content.slice(0, 300) + "..." : m.content}"`);
