...
`)}function B(e){return e in k}var U=8;var _=/<tool_call\s+name="(\w+)">\s*(\{.*?\})\s*<\/tool_call>/gs;function N(){return`You are a helpful research assistant with access to web search and page fetching.
...
You may output ONE tool_call per response, followed by a brief note.`}function D(e){let t=[],o;for(_.lastIndex=0;(o=_.exec(e))!==null;){let[,n,r]=o;try{let a=JSON.parse(r);B(n)&&t.push({name:n,args:a})}catch{console.warn(`[Agent] Failed to parse tool_call args: ${r}`)}}return t}function F(e){return e.replace(_,"").trim()}async function O(e,t,o=[],n,r){let s=N()+`

`;if(o.length>0){s+=`Recent conversation:
`;for(let g of o){let f=g.role==="user"?"User":"Assistant";s+=`${f}: ${g.content}
`}s+=`
`}s+=`User message: ${t}`;let u=0;for(;u<U;){u++,n?.(`Thinking... (step ${u})`);let f=(await e.generateText({instruction:s})).toString(),y=D(f);if(y.length===0)return F(f);for(let c of y){let x=$(c.name);if(x){n?.(`Using ${c.name}...`);try{let d=await x.execute(c.args);r?.(c.name,c.args,d);let l="";c.name==="web_search"?l="Analyze these search results. Pick the 1-2 most relevant URLs and use scrape_url to read their full content before answering.":c.name==="scrape_url"?l="You now have the page content. Give your FINAL answer based on this real data.":l="Now respond to the user based on this information.";let b=`

[Tool Result - ${c.name}]:
${d}

${l}`;s+=b}catch(d){let l=d instanceof Error?d.message:String(d);console.error(`[Agent] Tool ${c.name} failed:`,l),s+=`

[Tool Error - ${c.name}]: ${l}

The tool failed. Respond to the user explaining the issue.`}}}}return"I apologize, but I wasn't able to complete that task after multiple attempts."}var h=null;function C(e){h=e}function H(){return h?.thread?.customData?h.thread.customData:{}}function z(e){try{return H()[e]??void 0}catch(t){console.warn("[Storage] get("+e+") failed:",t);return}}function L(e,t){try{if(!h?.thread?.customData){console.warn("[Storage] set: oc.thread.customData not available");return}h.thread.customData[e]=t}catch(o){console.warn("[Storage] set("+e+") failed:",o)}}var i=window.oc,A=!1,W=10;function Y(){console.log("\u{1F916} Agent v0.1.0+0c1fc9f"),console.log("   Build: 2026-06-28 00:14:23"),console.log("   https://github.com/Fahell/agent-perchance")}var P="agent:jina_key";function J(){return z(P)??null}function K(e){L(P,e)}function q(){return i?i.thread?typeof i.generateText!="function"?(console.error("\u274C [Agent] oc.generateText not available"),!1):!0:(console.error("\u274C [Agent] oc.thread not available"),!1):(console.error("\u274C [Agent] window.oc not found \u2014 are you running inside Perchance?"),!1)}function G(){document.body.innerHTML=`
    <div style="font-family: system-ui; padding: 24px; background: #1a1a2e; color: #eee; height: 100vh; margin: 0; display: flex; align-items: center; justify-content: center;">
      <div style="max-width: 480px; width: 100%;">
        <div style="text-align: center; margin-bottom: 24px;">
          ...
          <span style="font-size: 11px; color: #666;">v0.1.0+0c1fc9f</span>
        </div>
    ...
    <div style="font-family: system-ui; padding: 8px; background: #1a1a2e; color: #eee; height: 100vh; margin: 0; display: flex; flex-direction: column; box-sizing: border-box; overflow: hidden;">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px; flex-shrink: 0;">
        ...
        <div style="display: flex; align-items: center; gap: 6px;">
          <span style="font-size: 10px; color: #666;">v0.1.0+0c1fc9f</span>
  ...
  `,document.body.appendChild(o),document.getElementById("settings-close").addEventListener("click",()=>o.remove()),document.getElementById("settings-save").addEventListener("click",async()=>{let n=document.getElementById("settings-key-input").value.trim(),r=document.getElementById("settings-msg");if(!n){r.textContent="Insira uma chave.",r.style.color="#f87171",r.style.display="block";return}r.textContent="Validando...",r.style.color="#aaa",r.style.display="block",await w(n)?(K(n),m(n),r.textContent="\u2705 Chave salva!",r.style.color="#4ade80",setTimeout(()=>o.remove(),1e3)):(r.textContent="\u274C Chave inv\xE1lida.",r.style.color="#f87171")})}function Z(e){let t=e.trim();return t.startsWith("/agent")||t.startsWith("/test-storage")}function Q(e){let t=e.trim();t==="/agent open"?(i.window.show(),console.log("\u{1FA9F} [Agent] Window opened")):t==="/agent close"?(i.window.hide(),console.log("\u{1FA9F} [Agent] Window closed")):t==="/test-storage"||t==="/test-storage run"?ee():t==="/test-storage check"?te():t==="/test-storage clean"&&(j(),p(`<div style="margin:8px 0;padding:8px;background:#16213e;border-radius:6px;border-left:3px solid #4ade80;">
  ...
  </div>`);let o=[{label:"1KB",bytes:1024},{label:"5KB",bytes:5*1024},{label:"10KB",bytes:10*1024},{label:"50KB",bytes:50*1024},{label:"100KB",bytes:100*1024},{label:"500KB",bytes:500*1024},{label:"1MB",bytes:1048576},{label:"5MB",bytes:5*1048576},{label:"10MB",bytes:10*1048576},{label:"25MB",bytes:25*1048576},{label:"50MB",bytes:50*1048576},{label:"100MB",bytes:100*1048576}];j();let n=[],r="";for(let{label:a,bytes:s}of o){p(`<div style="margin:2px 0 2px 12px;padding:4px 8px;color:#aaa;font-size:12px;">\u23F3 ${a} (${t(s)})...</div>`);let u="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",g="";for(;g.length<s;)g+=u;g=g.slice(0,s);let f=!1,y=0,c=!1,x=0;try{let l=performance.now();i.thread.customData[e+a]=g,y=Math.round(performance.now()-l),f=!0}catch(l){p(`<div style="margin:1px 0 1px 16px;padding:2px 8px;font-size:11px;color:#f87171;">
        \u274C WRITE FAILED: ${l instanceof Error?l.message:l}
      </div>`),n.push(`${a}\u274C`);break}try{let l=performance.now(),b=i.thread.customData[e+a];x=Math.round(performance.now()-l),c=b===g}catch{c=!1}let d=f&&c;if(p(`<div style="margin:1px 0 1px 16px;padding:2px 8px;font-size:11px;color:${d?"#4ade80":"#f87171"};">
      ${d?"\u2705":"\u274C"} Write: ${y}ms | Read: ${x}ms
