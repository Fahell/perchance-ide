...
[...truncated]`:r;return{url:e,title:a,content:o}}var d={web_search:{name:"web_search",description:"Search the web for current information. Returns titles, URLs, and descriptions.",parameters:{query:"The search query string"},execute:async e=>(await y(e.query,5)).raw||"No results found."},scrape_url:{name:"scrape_url",description:"Fetch and extract the main content from a URL as markdown.",parameters:{url:"The URL to scrape"},execute:async e=>{let t=await T(e.url,5e3);return`# ${t.title}

${t.content}`}}};function _(e){return d[e]}function b(){return Object.values(d).map(e=>`- ${e.name}: ${e.description}
  Parameters: ${JSON.stringify(e.parameters)}`).join(`
`)}function v(e){return e in d}var $=5,g=/<tool_call\s+name="(\w+)">\s*(\{.*?\})\s*<\/tool_call>/gs;function O(){return`You have access to the following tools:
...
User message: ${t}`,o=0;for(;o<$;){o++,n?.(`Thinking... (step ${o})`);let h=(await e.generateText({instruction:a})).toString(),m=S(h);if(m.length===0)return E(h);for(let c of m){let w=_(c.name);if(w){n?.(`Using ${c.name}...`);try{let l=await w.execute(c.args);r?.(c.name,c.args,l);let u=`
...
The tool failed. Respond to the user explaining the issue.`}}}}return"I apologize, but I wasn't able to complete that task after multiple attempts."}var s=window.oc;function I(){console.log("\u{1F916} Agent v0.1.0+23826d5"),console.log("   Build: 2026-06-27 01:52:02"),console.log("   https://github.com/Fahell/agent-perchance")}function P(){return s?s.thread?typeof s.generateText!="function"?(console.error("\u274C [Agent] oc.generateText not available"),!1):!0:(console.error("\u274C [Agent] oc.thread not available"),!1):(console.error("\u274C [Agent] window.oc not found \u2014 are you running inside Perchance?"),!1)}function M(){document.body.innerHTML=`
  ...
  `,s.window.show(),console.log("\u{1FA9F} [Agent] Window opened")}function k(e){return e.trim().startsWith("/agent")}function L(e){let t=e.trim();t==="/agent open"?(s.window.show(),console.log("\u{1FA9F} [Agent] Window opened")):t==="/agent close"&&(s.window.hide(),console.log("\u{1FA9F} [Agent] Window closed"))}function p(e){let t=document.getElementById("agent-output");t&&(t.innerHTML+=e,t.scrollTop=t.scrollHeight)}async function N(e){console.log("\u{1F916} [Agent] Processing:",e.content.slice(0,80)),p(`<div style="margin: 8px 0; padding: 8px; background: #16213e; border-radius: 6px; border-left: 3px solid #00d4ff;">
  ...
  </div>`)}function j(){I(),console.log("\u{1F680} [Agent] Loading..."),M(),s.messageRenderingPipeline.push(({message:e,reader:t})=>{t==="ai"&&e.author==="user"&&(e.expectsReply=!1,e.hiddenFrom=["ai"],console.log("\u{1F6E1}\uFE0F [Agent] Pipeline: blocked AI from seeing user message"))}),s.thread.on("MessageAdded",function({message:e}){if(e.author==="user"){if(k(e.content)){L(e.content),setTimeout(()=>{let t=s.thread.messages.indexOf(e);t!==-1&&s.thread.messages.splice(t,1)},100);return}console.log("\u{1F4E8} [Agent] Processing:",e.content.slice(0,80)),N(e).catch(t=>{console.error("\u274C [Agent] Error:",t),s.thread.messages.push({author:"ai",content:`Sorry, I encountered an error: ${t instanceof Error?t.message:String(t)}`})})}}),console.log("\u2705 [Agent] Ready!")}P()&&j();
