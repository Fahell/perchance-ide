...
`}s+=`
`}s+=`User message: ${t}`;let u=0;for(;u<D;){u++,n?.(`Thinking... (step ${u})`);let f=(await e.generateText({instruction:s})).toString(),x=F(f);if(x.length===0)return U(f);for(let l of x){let y=I(l.name);if(y){n?.(`Using ${l.name}...`);try{let c=await y.execute(l.args);o?.(l.name,l.args,c);let d=`
