...
export async function getMemories() {
    const memories = await dbKvGet(MEMORIES_KEY);
    if (!Array.isArray(memories))
        return [];
    return memories.filter((m) => typeof m === "string");
}
...
// ─── Memory Extraction ──────────────────────────────────────
export async function extractMemories(userMessage, agentResponse) {
...
New facts (one per line, or NONE):`;
    try {
        ...
        const unique = newFacts.filter((fact) => !existing.some((e) => e.toLowerCase() === fact.toLowerCase()));
        if (unique.length === 0) {
