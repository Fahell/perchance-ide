...
function findOccurrences(content: string, search: string): Occurrence[] {
  ...
  return {
    ...
    edit_file: {
      ...
      execute: async (args) => {
        ...
        if (!filePath.startsWith("/")) return "Error: path must be absolute (start with /).";

        if (!vfsExists(filePath)) return `Error: File not found: ${filePath}`;
        const content = vfsRead(filePath);
        if (content === null) return `Error: ${filePath} is a directory, not a file.`;
